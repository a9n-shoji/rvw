...
- コメントと返信のsafe GFM表示（repository内link／同一commit相対画像、表示専用Mermaid、
  global対象commitへ安全に配置し、できなければexact commitへfallbackするinline code referenceを含む）
...
globalなcommit選択を変えずに開けます。global対象commitへ安全に配置できるreferenceは現在の比較で直接開き、
内容変更や削除などで配置できない場合は投稿時点のexact sourceへfallbackします。
