CREATE TABLE comment_targets_v3 (
  ...
  CHECK(
    ...
    OR
    (target_kind = 'walkthrough' AND document_kind IS NULL AND source_oid IS NULL AND file_path IS NULL AND source_document_hash IS NULL AND quoted_text IS NULL AND walkthrough_id IS NOT NULL AND start_line IS NULL AND end_line IS NULL)
  ),
  CHECK((start_line IS NULL AND end_line IS NULL) OR (start_line IS NOT NULL AND end_line IS NOT NULL))
);

INSERT INTO comment_targets_v3(
  comment_id,
  target_kind,
  document_kind,
  source_oid,
...
ALTER TABLE comment_targets_v3 RENAME TO comment_targets;
