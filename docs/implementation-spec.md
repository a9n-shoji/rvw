...
評価する。そこまで同点のcandidateは、同じwidth内のlocal rank load profileと、directed in/out degreeおよびboundary roleを
初期色としてincoming/outgoing neighbor colorを反復した軽量なstructural profileで比較し、その後にだけstable Node IDを使う。
