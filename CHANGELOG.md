...
- コメント対象とコメント内のcode参照を、現在選択中のcommitへ安全に配置できる場合はその比較documentで直接開き、変更・削除・曖昧な場合は投稿時点のsourceへfallbackするようにした

### Fixed

- 承認後にコメントのみのreviewが追加されたPull Requestで、GitHub上の現在の承認数が0件として表示される問題を修正
