...
Start from the explicit subject and authority supplied by the user, caller, and Pull Request body.
...
`mustEstablish` limited to facts needed for the central review question rather than using it as a
...
mustEstablish: claims the path must ground in source
...
origin: factual code entrypoint, when already established; otherwise producer must verify it
...
mustEstablish: node, relation, and boundary claims that need source evidence
