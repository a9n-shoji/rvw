...
`presentation` is required nullable version-6 content. Use `null` when the factual graph is clearest
...
Keep factual graph claims, authorial presentation, derived rendering, and reviewer session state
...
- Choose one current `startNodeId` as the first authorial attention anchor. It may differ from
  ...
  must be one of its derived endpoint Nodes.
- Use `primaryBackbone: null` when this explanation has no compact connected relation skeleton worth
  emphasizing.
  A meaningful thesis and start alone form a valid start-only presentation. A singleton map may use
  this form or `presentation: null`; never invent an Edge to decorate it.
...
The Viewer provides Reset, Fit, zoom, and pan; these are reviewer concerns rather than authoring inputs.
...
- [ ] A one-file map with no meaningful inter-file dependency has one Node and zero Edges; no dummy Edge,
      dependency or backbone was created.
