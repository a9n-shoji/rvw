...
describe("rvw-watch-comments delegation contract", () => {
  ...
  it("parallelizes investigate-only follow-ups without weakening writer serialization", () => {
    ...
    expect(skill).toMatch(
      /Repository Review leases are always read-only[\s\S]*same-repository follow-ups/,
    );
  });

  it("keeps Repository Review outcomes read-only while allowing exact typed evidence", () => {
    expect(skill).toMatch(
      /For a Repository Review outcome it\s+must be the current or an already retained Repository Review source commit/,
    );
    expect(skill).toMatch(
      /For every concrete claim about code behavior[\s\S]*typed references by default/,
    );
    expect(skill).not.toContain("Repository Review outcomes always use null, no references");
  });
});
