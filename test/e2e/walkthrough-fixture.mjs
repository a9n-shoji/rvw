...
const mainReferences = [
  ...
  reference(
    ...
    "async runWithClient",
    14,
    "Atomic persistence on the idempotency lock connection",
  ),
  reference(
    "repository",
...
export const walkthroughs = [
  {
    ...
    body: markdown(
      ...
      "",
      "DB 内では idempotency store が lock 済みconnectionを [TransactionRunner.runWithClient](rvw-ref:transaction) へ渡し、[PostgresOrderRepository.insert](rvw-ref:repository) と [PostgresOutbox.append](rvw-ref:outbox) を一緒に commit します。これによりpool capacityが1でも追加connectionを待たず、『注文だけ保存され event が消える』状態も作りません。",
      "",
      "## 4. commit 後の非同期処理",
      "",
  ...
  },
  {
    ...
    body: markdown(
      ...
      "",
      "決済成功後に [TransactionRunner.runWithClient](rvw-ref:transaction) が rollback したケースは [PaymentReconciliationWorker](rvw-ref:reconcile) が orphan authorization を void します。DB commit 後の event は [PostgresOutbox.append](rvw-ref:outbox) と [OutboxDispatcher.tick](rvw-ref:dispatcher) が引き継ぎます。",
      "",
  ...
  },
  {
    ...
    body: markdown(
      ...
      "4. [TransactionRunner.runWithClient](rvw-ref:transaction) の中で order と [PostgresOutbox.append](rvw-ref:outbox) を永続化する。",
