...
const scenarioName = process.env.RVW_FIXTURE_SCENARIO ?? "contract";
if (!new Set(["contract", "realistic", "dogfood"]).has(scenarioName)) {
  throw new Error(
    `unknown RVW_FIXTURE_SCENARIO ${JSON.stringify(scenarioName)}; expected contract, realistic, or dogfood`,
  );
}
const repositoryFixture =
  scenarioName === "realistic"
    ? (await import("../fixtures/realistic/realistic-fixture.mjs")).createRealisticFixture()
    : scenarioName === "dogfood"
      ? (await import("../../scripts/dogfood-fixture.ts")).createDogfoodFixture(
          path.resolve(import.meta.dirname, "../.."),
        )
      : null;
const pullRequestId = repositoryFixture?.pullRequestId ?? "11111111-1111-4111-8111-111111111111";
...
const baseOid = repositoryFixture?.baseOid ?? "a".repeat(40);
const firstHead = repositoryFixture?.commits[0]?.oid ?? "b".repeat(40);
const secondHead = repositoryFixture?.headOid ?? "c".repeat(40);
const comments = repositoryFixture ? structuredClone(repositoryFixture.comments) : [];
const activeWalkthroughs = repositoryFixture
  ? structuredClone(repositoryFixture.walkthroughs)
  : structuredClone(walkthroughs);
...
function currentPullRequest() {
  if (repositoryFixture) return repositoryFixture.pullRequest;
...
function currentView() {
  if (repositoryFixture) {
    const pullRequest = repositoryFixture.pullRequest;
    return {
      pullRequest,
      pullRequestContentFingerprint: pullRequestContentFingerprint(pullRequest),
      comparisonBaseOid: repositoryFixture.baseOid,
      headOid: repositoryFixture.headOid,
      commits: repositoryFixture.commits,
      changeSequence,
      revisions,
    };
  }
...
function repositoryDocumentText(oid, filePath) {
  if (repositoryFixture) return repositoryFixture.repositoryDocumentAt(oid, filePath).text ?? "";
...
function repositoryPathsAt(oid) {
  if (repositoryFixture) {
    return repositoryFixture.repositoryEntriesAt(oid).map((entry) => entry.path);
  }
  return [
    ...new Set([
      "README.md",
...
function repositoryDocument(ref) {
  if (repositoryFixture) {
    return { ref, ...repositoryFixture.repositoryDocumentAt(ref.sourceOid, ref.path) };
  }
...
app.get("/api/pull-requests", (context) => {
  ...
  const statusFixtureItems = repositoryFixture
    ? [
        {
          pullRequestId,
          owner: "a9n-shoji",
...
app.post("/api/pull-requests/:id/refresh", async (context) => {
  ...
  if (!repositoryFixture) syncStage = Math.min(syncStage + 1, 2);
...
app.get("/api/pull-requests/:id/tree", (context) => {
  ...
  if (repositoryFixture) {
    return context.json({
      ...
      entries: repositoryFixture.repositoryEntriesAt(oid),
    });
  }
...
app.get("/api/pull-requests/:id/changed-files", (context) => {
  if (repositoryFixture) {
    ...
    return context.json({
      ...
      oldOid,
      newOid,
      files: repositoryFixture.changedFiles(oldOid, newOid),
    });
  }
...
function fixtureCommentPlacement(comment, destination) {
  ...
  const destinationOid =
    destination.kind === "commit"
      ? destination.oid
      : requestedRef?.kind === "repository-file"
        ? requestedRef.sourceOid
        : null;
  const targetPath =
    comment.target.documentKind === "repository-file" &&
    destinationOid &&
    repositoryFixture?.resolvePathAt
      ? repositoryFixture.resolvePathAt(
  ...
  const pathMatches = targetPath !== null && (!requestedPath || requestedPath === targetPath);
  let range =
  ...
  return {
    ...
    path: targetPath ?? originalTargetPath,
  };
}
