...
test("keeps the chosen center anchored across depths and makes local Fit camera-only", async ({
  ...
  const aggregateBeforeDepth = await nodeScreenCenter(orderAggregate);
  ...
  expectSamePoint(await nodeScreenCenter(orderAggregate), aggregateBeforeDepth);
  expect(
    await orderAggregate.evaluate((element) => ({
      left: (element as HTMLElement).style.left,
      top: (element as HTMLElement).style.top,
    })),
  ).toEqual(aggregateLocalPosition);
...
test("continues wheel and pointer pans from the camera's interpolated position", async ({
  ...
  expect(wheelResult.x).toBeCloseTo(wheelStart.x - 62, 1);
  expect(wheelResult.y).toBeCloseTo(wheelStart.y - 34, 1);
  ...
  const panStart = {
    x: canvasBounds!.x + 12,
    y: canvasBounds!.y + canvasBounds!.height - 12,
  };
