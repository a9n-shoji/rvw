...
function projectPresentedStructure(
  ...
  if (presentation.primaryBackbone === null) return null;
  ...
  if (backboneEdges.length === 0) return null;
  const positions = new Map<string, StructurePoint>();
  {
    ...
    for (const rank of rankValues) {
      ...
      group.sort((left, right) => {
        ...
        return previousBarycenter(left) - previousBarycenter(right) || stableCompare(left, right);
      });
  ...
  const fallbackX =
    Math.max(0, ...[...positions.values()].map(({ x }) => x + STRUCTURE_NODE_WIDTH)) +
